...
export function BatteryExplainer({ battery }: { battery: BatteryBreakdown }) {
  return (
    <div className="rounded-xl border border-line bg-navy-900 p-4">
      ...
      <p className="text-xs text-mute">confident range after the safety reserve</p>
      <div className="mt-3 overflow-x-auto rounded-lg bg-navy-950 p-3 font-mono text-[11px] leading-relaxed text-mute">
        ...
        <div>adjusted Wh/km = {battery.baseConsumptionWhPerKm} × {battery.terrainFactor.toFixed(2)} terrain × {battery.trafficFactor.toFixed(2)} traffic × {battery.weatherFactor.toFixed(2)} weather = {battery.adjustedWhPerKm}</div>
